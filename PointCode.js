...
if (usingWebAudio && Point.context && Point.context.state === 'suspended') {
  var resume = function () {
...
window.addEventListener("resize", function() {
	...
	if(Point.orientation !== Point.orientPrev) {
...
if(!Point.scoreMode) {
	Point.scoreMode = Point.scoreModeList[randInt(0,Point.scoreModeList.length)]
}
Point.scoreModeCurrent = Point.scoreMode;
for (var i = 0; i < Point.scoreModeList.length; i++) {
	Point.scoreModeListExt.push({
		name: Point.scoreModeList[i],
		nbrBoxes: [],
		colors: []
	})
}
for (var x = 0; x < Point.nbrX; x++) {
	for (var y = 0; y < Point.nbrY; y++) {
		Point.tuples.push([x,y]);
	}
}
...
Point.close = function(){
	...
	if(Point.switchValue === 0) {
		Point.closeGUI = setTimeout(Point.switch,10*1000)
...
Point.init = function() { 
	...
	Point.mainTask = Point.taskConstructor(function() { 
		...
		if(Point.waitChanged) {
			wait = Point.waitOrig;
			Point.waitChanged = false;
		}
		
		// reset if scoreModeChanged
		if(Point.scoreModeChanged) {
			Point.scoreModeChanged = false;
		}
		if(Point.scoreShuffled) {
			Point.scoreShuffled = false;
		}
		
		// set new wait
		if(wait < 0.5 * Point.mainWaitDefault) {
			Point.waitSign = 1;
		}
		if(wait > 2 * Point.mainWaitDefault) {
			Point.waitSign = -1;
		}
		wait = wait + Point.waitSign * Point.mainWaitChange * Math.random();
		if(Math.random() < 0.1 || Point.fastForward) { 
			Point.waitOrig = wait; 
			if (wait > Point.mainWaitDefault) {
				wait = wait / 2;
			} else {
				wait = wait * 2;
			}
...
Point.shuffleScore = function() { 
	...
	Point.scoreModeListExt[modeIndex].nbrBoxes = 
		fillArray(4, 7, 1, 1)
			.concat(fillArray(7, 12, 3, 6)
				.concat(fillArray(1, 2, 1, 1)
					.concat(fillArray(2, 4, Point.nbrX * Point.nbrY / 2, Point.nbrX * Point.nbrY)
						.concat(fillArray(6,10,1,3)
						)
					)
				)
		);
	...
	case 'grey':
		...
		break;
	case 'mellow':
		for (i = 0; i < randInt(25,35); i++) { // mellow colors
	...
	case 'squares':
		...
		break;
	case 'lines':
		index = 0; // to keep track of position in array
		for (i = 0; i < randInt(1,4); i++) { // black or white
			if (Math.random()<0.5) {
				Point.scoreModeListExt[modeIndex].colors.push([ [255,255,255], [255,255,255] ])
			} else {
				Point.scoreModeListExt[modeIndex].colors.push([ [0,0,0], [0,0,0] ])
			}
			index += 1;
		}
		for (i = 0; i < randInt(3,6); i++) { // pure colors
	...
	default:
		index = 0; // to keep track of position in array
		for (i = 0; i < randInt(15,25); i++) { // all colors
			Point.scoreModeListExt[modeIndex].colors.push([ [0,0,0], [255,255,255] ]);
			index += 1;
		}
		for (i = 0; i < randInt(5,15); i++) { // mellow colors
			Point.scoreModeListExt[modeIndex].colors.push([ fillArray(3,3,85,153), [255,255,255] ]);
			for (j = 0; j < 3; j++) {
				Point.scoreModeListExt[modeIndex].colors[index][1][j] =
					Point.scoreModeListExt[modeIndex].colors[index][0][j] + randInt(10,25);
			}
			index += 1;
		}
		for (i = 0; i < randInt(5,8); i++) { // hard colors high
		...
		for (i = 0; i < randInt(5,8); i++) { // hard colors low
...
Point.drawBoxes = function(fadeTime) { // fadeTime in seconds
	var rgb = new Array(3), rgbText = new Array(3), red, green, blue, tuple = [], tmp, x, y;
	...
	switch(Point.scoreMode) {
		case 'single':
			...
			if(Point.scoreModeChanged || Point.scoreShuffled) {
				Point.tuplesTemp = Point.tuples;
				shuffleArray(Point.tuplesTemp);
				Point.tuplesTemp = Point.tuplesTemp.filter( function(value, index) {
					if(index < randInt(2,4)) {return value}
				});
			} else {
			...
			break;
		case 'squares':
			x = randInt(0,Point.nbrX - 2);
			y = randInt(0,Point.nbrY - 2);
			nbr = 4;
			Point.tuplesTemp = Point.tuplesTemp.filter(function(value)
				{
	...
	for (var index = 0; index < nbr; index += 1) {
		...
		for (var i = 0; i < 3; i++) {
			rgb[i] = randInt(
				Point.colors[Point.count % Point.colors.length][0][i],
